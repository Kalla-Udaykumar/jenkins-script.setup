...
pipeline{
    ...
    //Downstream QA Job inclusive of BDBA and Virus Scan
        ...
        PACKAGE_DOCKER_IMAGE = "amr-registry.caas.intel.com/esc-devops/gen/lin/edge/ubuntu/package_builder_noble:20240325_1700"
    ...
    parameters {
        string(name: 'MANIFEST_GIT_REPO', description: 'Set this value if you wish to change the default manifest repo', defaultValue: 'https://github.com/intel-innersource/os.linux.ubuntu-integration.build-automation.git')
        ...
        string(name: 'MANIFEST_FILE', description: 'Set this value if you wish to change the default manifest file', defaultValue: 'noble.json')
        string(name: 'KERNELS_FILE', description: 'Path for the kernels json file, eg: manifest/kernels.json', defaultValue: '')
        string(name: 'EXTRA_DEBS', description: 'Extra debs file path, eg: manifest/jammy-extra-debs.json', defaultValue: 'manifest/noble-extra-debs.json')
        string(name: 'DISTRO_NAME', description: 'Set this value if you wish to change the distro name used in the artifactory path', defaultValue: 'noble')
                ...
                script{
                    ...
                    for (item in props){
                        ...
                        sh"""
                            ...
                            for file in *.changes; do reprepro -b ${WORKSPACE}/PPA -C ${component} include noble "\$file"; done'
                        """
                    }
                }
            }
                    ...
                    if(!params.EXTRA_DEBS.isEmpty()){
                        ...
                        for (components in extra_debs){
                            ...
                            sh"""
                                ...
                                rm -rf ${WORKSPACE}/PPA/db/lockfile || true
                                $PACKAGE_SIGN_DOCKER bash -c 'cd ${WORKSPACE}/PPA && \
                                reprepro -C ${component} includedeb noble *.deb'
                            """
                        }
                    }
                    // ########################################################
            ...
            steps{
                ...
                dir("${WORKSPACE}/PPA/upload") {
                                sh"""
                                    ...
                                    curl -u ${BDUSR}:${BDPWD} -v -X DELETE https://ubit-artifactory-sh.intel.com/artifactory/esc-internal-local/sandbox/ppa/noble-latest/ || true
                                """
                                ...
                                def kwrpt  = """{
                                    "files": [
                                        ...
                                        {
                                            "pattern": "*",
                                            "target": "esc-internal-local/sandbox/ppa/noble-latest/",
                                            "props": "retention.days=3",
                                            "flat" : "false"
                                        }
                                    ]
                            ...
                            curl -u ${BDUSR}:${BDPWD} -v -X PUT -T build-info https://ubit-artifactory-sh.intel.com/artifactory/esc-internal-local/sandbox/ppa/noble-latest/build-info
                            """
                        }
                        else{
